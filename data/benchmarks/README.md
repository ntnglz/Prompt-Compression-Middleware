...
| [e2e-prompts-corpus.md](../docs/e2e-prompts-corpus.md) | Dataset extenso desde transcripts COE anonimizados |
...
# E2E standalone — compresor fine-tuned (usar reasoning=none)
python scripts/e2e_benchmark.py --compressor-model pcm-granite --reasoning-effort none -q

# Corpus extenso — prompts largos anonimizados desde COE transcripts
python scripts/e2e_benchmark.py --compressor-model pcm-granite \
  --prompts data/e2e_prompts_extensive.json --reasoning-effort none -q

# Baseline compresor sin fine-tune (solo comparativa)
python scripts/e2e_benchmark.py --compressor-model granite4.1:3b --reasoning-effort none -q
```
